...
/**
 * AbeBooks – Search for used/rare/first edition books
 */

const CFG = {
  searchQuery: "To Kill a Mockingbird first edition",
  maxResults: 5,
...
AbeBooks – Used/rare book search
...
(async () => {
  ...
  try {
    ...
    const url = `https://www.abebooks.com/servlet/SearchResults?kn=${encoded}&sortby=17`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.goto(url);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} book listings. For each get the title, author, edition, condition, seller, and price.`
    );
    recorder.record("extract", { description: "book listings", results: data });
    console.log("Extracted:", JSON.stringify(data, null, 2));

    const outDir = __dirname;
    fs.writeFileSync(path.join(outDir, "recorded_actions.json"), JSON.stringify(recorder.actions, null, 2));
    fs.writeFileSync(path.join(outDir, "abebooks_search.py"), genPython(CFG, recorder));
    console.log("Saved abebooks_search.py");
  } finally {
