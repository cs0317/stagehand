...
(async () => {
  ...
  try {
    ...
    const url = `https://www.surfline.com/search/${query}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} surf spot results. For each get the spot name, location, wave height, wind conditions, water temp, and rating.`
    );
    recorder.record("extract", "spot listings", JSON.stringify(data));
    console.log("Extracted:", JSON.stringify(data, null, 2));

    const outDir = __dirname;
    fs.writeFileSync(path.join(outDir, "recorded_actions.json"), JSON.stringify(recorder.actions, null, 2));
    fs.writeFileSync(path.join(outDir, "surfline_search.py"), genPython(CFG, recorder));
    console.log("Saved surfline_search.py");
  } finally {
