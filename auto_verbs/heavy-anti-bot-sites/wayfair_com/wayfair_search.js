...
const CFG = { url: "https://www.wayfair.com", query: "standing desk", maxResults: 5, waits: { page: 3000, type: 2000, search: 5000 } };

function genPython(cfg, recorder) { const ts = new Date().toISOString();
  return `"""Auto-generated – Wayfair Product Search. Query: ${cfg.query}. Generated: ${ts}"""
...
def run(playwright: Playwright, query: str = "${cfg.query}", max_results: int = ${cfg.maxResults}) -> list:
    print(f"  Query: {query}\\n")
    port = get_free_port(); profile_dir = get_temp_profile_dir("wayfair_com")
    ...
    try:
        ...
        si = page.locator('input[id="sbx-primary"], input[data-hb-id="search-bar"], input[aria-label*="search" i], input[type="search"]').first
        ...
        try:
            sort_btn = page.locator('select[data-hb-id*="sort"], button:has-text("Sort"), [class*="SortDropdown"], select[id*="sort"]').first
            sort_btn.select_option(label="Best Sellers") if sort_btn.evaluate("el => el.tagName") == "SELECT" else sort_btn.click()
            page.wait_for_timeout(2000)
        except Exception:
            try:
                page.locator('button:has-text("Sort"), [class*="sort"]').first.click()
                page.wait_for_timeout(500)
                page.locator('a:has-text("Best Sellers"), li:has-text("Best Sellers"), option:has-text("Best Sellers")').first.click()
                page.wait_for_timeout(2000)
            except Exception: pass
        cards = page.locator('[data-hb-id*="product-card"], a[class*="ProductCard"], div[class*="ProductCard"], div[class*="product-card"]')
        count = cards.count()
        for i in range(min(count, max_results)):
            card = cards.nth(i); name = price = rating = "N/A"
            try: name = card.locator('[data-hb-id*="product-name"], [class*="ProductCard-name"], [class*="product-name"], h2, p[class*="title"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: price = card.locator('[class*="price"], span:has-text("$")').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: rating = card.locator('[class*="rating"], [class*="stars"], [aria-label*="star" i]').first.get_attribute("aria-label", timeout=2000) or card.locator('[class*="rating"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            if name != "N/A": results.append({"name": name, "price": price, "rating": rating}); print(f"  {len(results)}. {name} | {price} | {rating}")
        print(f"\\nFound {len(results)} products:")
        for i, r in enumerate(results, 1): print(f"  {i}. {r['name']} — {r['price']} (Rating: {r['rating']})")
...
async function main() { const recorder = new PlaywrightRecorder(); const llmClient = setupLLMClient("hybrid"); let stagehand;
  try { stagehand = new Stagehand({ env: "LOCAL", verbose: 0, llmClient, localBrowserLaunchOptions: { userDataDir: path.join(os.homedir(), "AppData", "Local", "Google", "Chrome", "User Data", "Default"), headless: false, viewport: { width: 1920, height: 1080 }, args: ["--disable-blink-features=AutomationControlled", "--disable-infobars", "--disable-extensions", "--start-maximized"] } });
    ...
    await observeAndAct(stagehand, page, recorder, "Sort by Best Sellers", "Sort");
    await page.waitForTimeout(CFG.waits.page);
    const { z } = require("zod/v3");
    const listings = await stagehand.extract(`Extract up to ${CFG.maxResults} products with name, price, and customer rating.`, z.object({ products: z.array(z.object({ name: z.string(), price: z.string(), rating: z.string() })) }));
    recorder.record("extract", { instruction: "Extract products", results: listings });
    fs.writeFileSync(path.join(__dirname, "wayfair_search.py"), genPython(CFG, recorder), "utf-8");
