...
def viator_search(page: Page, request: ViatorSearchRequest) -> ViatorSearchResult:
    ...
    url = f"https://www.viator.com/search/{query}"
    print(f"Loading {url}...")
...
(async () => {
  ...
  try {
    ...
    const url = `https://www.viator.com/search/${query}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} tour/activity results. For each get the tour name, price, duration, rating, number of reviews, location, and highlights.`
    );
    recorder.record("extract", "tour listings", JSON.stringify(data));
    console.log("Extracted:", JSON.stringify(data, null, 2));

    const outDir = __dirname;
    fs.writeFileSync(path.join(outDir, "recorded_actions.json"), JSON.stringify(recorder.actions, null, 2));
    fs.writeFileSync(path.join(outDir, "viator_search.py"), genPython(CFG, recorder));
    console.log("Saved viator_search.py");
  } finally {
