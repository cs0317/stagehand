...
(async () => {
  ...
  try {
    ...
    const url = `https://dogtime.com/?s=${query}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} dog breed search results. For each get the breed name, size, energy level, grooming needs, good with kids rating, and description.`
    );
    recorder.record("extract", "breed listings", JSON.stringify(data));
    console.log("Extracted:", JSON.stringify(data, null, 2));

    const outDir = __dirname;
    fs.writeFileSync(path.join(outDir, "recorded_actions.json"), JSON.stringify(recorder.actions, null, 2));
    fs.writeFileSync(path.join(outDir, "dogtime_search.py"), genPython(CFG, recorder));
    console.log("Saved dogtime_search.py");
  } finally {
