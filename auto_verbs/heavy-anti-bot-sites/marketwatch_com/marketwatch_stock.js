...
/**
 * MarketWatch – Stock Lookup
 * Navigate to a stock quote page and extract price, change, market cap, P/E ratio.
 */
...
function genPython(cfg, recorder) {
  ...
  return `"""Auto-generated – MarketWatch Stock Lookup. Ticker: ${cfg.ticker}. Generated: ${ts}"""
import re, os, sys, shutil
from playwright.sync_api import Playwright, sync_playwright
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))
from cdp_utils import get_free_port, get_temp_profile_dir, launch_chrome, wait_for_cdp_ws

def run(playwright: Playwright, ticker: str = "${cfg.ticker}") -> dict:
    ...
    result = {}
    try:
        page.goto(f"${cfg.url}/investing/stock/{ticker.upper()}")
        ...
        # Extract key data (market cap, P/E) from the aside/key data table
        try:
            key_data = page.locator('[class*="key-data"], [class*="profile"], ul[class*="list--kv"]')
            text = key_data.first.inner_text(timeout=3000)
            import re as _re
            mc = _re.search(r"Market Cap.*?([\\\\d\\\\.]+[BMT]?)", text)
            pe = _re.search(r"P/E Ratio.*?([\\\\d\\\\.]+)", text)
...
async function main() {
  ...
  try {
    ...
    recorder.goto(`${CFG.url}/investing/stock/${CFG.ticker}`);
    await page.goto(`${CFG.url}/investing/stock/${CFG.ticker}`);
    await page.waitForLoadState("domcontentloaded"); await page.waitForTimeout(CFG.waits.page);

    const { z } = require("zod/v3");
    const data = await stagehand.extract(
      `Extract the stock quote data for ${CFG.ticker}: current price, price change (amount and percent), market cap, and P/E ratio.`,
      z.object({ price: z.string(), change: z.string(), changePercent: z.string(), marketCap: z.string(), peRatio: z.string() })
    );
    recorder.record("extract", { instruction: "Extract stock data", results: data });
    console.log("📊 Stock data:", data);

    fs.writeFileSync(path.join(__dirname, "marketwatch_stock.py"), genPython(CFG, recorder), "utf-8");
