...
const CFG = { url: "https://www.nordstrom.com", query: "men's dress shoes", maxResults: 5, waits: { page: 3000, type: 2000, search: 5000 } };

function genPython(cfg, recorder) { const ts = new Date().toISOString();
  return `"""Auto-generated – Nordstrom Product Search. Query: ${cfg.query}. Generated: ${ts}"""
...
def run(playwright: Playwright, query: str = "${cfg.query}", max_results: int = ${cfg.maxResults}) -> list:
    print(f"  Query: {query}\\n")
    port = get_free_port(); profile_dir = get_temp_profile_dir("nordstrom_com")
    ...
    try:
        ...
        si = page.locator('input[id="keyword-search-input"], input[name="keyword"], input[aria-label*="search" i], input[type="search"]').first
        ...
        cards = page.locator('[data-testid*="product"], article[class*="product"], div[class*="product-card"]')
        count = cards.count()
        for i in range(min(count, max_results)):
            card = cards.nth(i); name = brand = price = "N/A"
            try: name = card.locator('[class*="product-title"], [data-testid*="title"], a[class*="name"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: brand = card.locator('[class*="brand"], [data-testid*="brand"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
...
async function main() { const recorder = new PlaywrightRecorder(); const llmClient = setupLLMClient("hybrid"); let stagehand;
  try { stagehand = new Stagehand({ env: "LOCAL", verbose: 0, llmClient, localBrowserLaunchOptions: { userDataDir: path.join(os.homedir(), "AppData", "Local", "Google", "Chrome", "User Data", "Default"), headless: false, viewport: { width: 1920, height: 1080 }, args: ["--disable-blink-features=AutomationControlled", "--disable-infobars", "--disable-extensions", "--start-maximized"] } });
    ...
    fs.writeFileSync(path.join(__dirname, "nordstrom_search.py"), genPython(CFG, recorder), "utf-8");
