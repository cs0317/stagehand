...
def polygon_search(page: Page, request: PolygonSearchRequest) -> PolygonSearchResult:
    ...
    js_code = """(max) => {
        const cards = document.querySelectorAll('article, [class*="SearchResult"], [class*="result"], [class*="Card"], [class*="story"], .c-entry-box--compact');
        const items = [];
...
(async () => {
  ...
  try {
    ...
    const url = `https://www.polygon.com/search?q=${query}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} gaming/entertainment article results. For each get the title, author, publish date, category, and summary.`
    );
    ...
    fs.writeFileSync(path.join(outDir, "polygon_search.py"), genPython(CFG, recorder));
    console.log("Saved polygon_search.py");
  } finally {
