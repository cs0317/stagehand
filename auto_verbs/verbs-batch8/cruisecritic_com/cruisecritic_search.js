...
def cruisecritic_search(page: Page, request: CruisecriticSearchRequest) -> CruisecriticSearchResult:
    ...
    url = f"https://www.cruisecritic.com/search/results.cfm?search={query}"
    print(f"Loading {url}...")
...
(async () => {
  ...
  try {
    ...
    const url = `https://www.cruisecritic.com/search/results.cfm?search=${query}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} cruise deal results. For each get the cruise line, ship name, destination, duration, departure date, price from, and rating.`
    );
    recorder.record("extract", "cruise deal listings", JSON.stringify(data));
    console.log("Extracted:", JSON.stringify(data, null, 2));

    const outDir = __dirname;
    fs.writeFileSync(path.join(outDir, "recorded_actions.json"), JSON.stringify(recorder.actions, null, 2));
    fs.writeFileSync(path.join(outDir, "cruisecritic_search.py"), genPython(CFG, recorder));
    console.log("Saved cruisecritic_search.py");
  } finally {
