...
/**
 * Amazon Warehouse – Search warehouse deals by category
 */
...
def amazon_warehouse_search(page: Page, request: WarehouseDealRequest) -> WarehouseDealResult:
    ...
    url = f"https://www.amazon.com/s?k={encoded}&i=warehouse-deals"
    print(f"Loading {url}...")
    checkpoint("Navigate to Amazon Warehouse deals")
...
(async () => {
  ...
  try {
    ...
    const url = `https://www.amazon.com/s?k=${encoded}&i=warehouse-deals`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} warehouse deal products. For each get the product name, original price, warehouse price, condition, and discount percentage.`
    );
    recorder.record("extract", "warehouse deals", JSON.stringify(data));
    console.log("Extracted:", JSON.stringify(data, null, 2));

    const outDir = __dirname;
    fs.writeFileSync(path.join(outDir, "recorded_actions.json"), JSON.stringify(recorder.actions, null, 2));
    fs.writeFileSync(path.join(outDir, "amazon_warehouse_search.py"), genPython(CFG, recorder));
    console.log("Saved amazon_warehouse_search.py");
  } finally {
