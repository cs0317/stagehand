...
def ocw_search(page: Page, request: CourseRequest) -> CourseResult:
    ...
    courses_data = page.evaluate(r\\"\\"\\"(maxResults) => {
        ...
        for (const a of links) {
            ...
            if (/^(search|home|sign|menu|filter|sort|about)/i.test(title)) continue;
            seen.add(title);
...
(async () => {
  ...
  try {
    const searchUrl = `https://ocw.mit.edu/search/?q=${encodeURIComponent(CFG.query)}&t=course`;
    console.log(`\\n🌐 Loading: ${searchUrl}...`);
    await page.goto(searchUrl, { waitUntil: "domcontentloaded", timeout: 30000 });
    await page.waitForTimeout(CFG.waits.page);
    recorder.record("goto", { url: searchUrl, description: "Navigate to MIT OCW search" });

    const courses = await stagehand.extract(
      \`extract up to \${CFG.maxResults} courses with title, course number, instructor, and URL\`
    );
    console.log("\\n📊 Courses:", JSON.stringify(courses, null, 2));
    recorder.record("extract", { instruction: "Extract courses", results: courses });
    ...
    fs.writeFileSync(path.join(dir, "ocw_search.py"), pyCode);
