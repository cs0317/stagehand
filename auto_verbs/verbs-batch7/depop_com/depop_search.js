 ...
 * Depop – Item Search
 *
 * Searches Depop for items and extracts:
 * item_name, brand, price, size, seller.
 */
...
(async () => {
  ...
  try {
    ...
    const url = `https://www.depop.com/search/?q=${q}`;
    console.log(`Navigating to ${url}`);
    await page.goto(url, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(CFG.waits.page);

    const result = await stagehand.extract({
      instruction: `Extract the first ${CFG.maxResults} listings. For each get: item name, brand, price, size, and seller username.`,
      schema: {
        type: "object",
        properties: {
          listings: {
    ...
    fs.writeFileSync(path.join(__dirname, "depop_search.py"), pyCode);
    console.log("\nSaved depop_search.py");
  } finally {
