...
const CFG = { url: "https://www.npr.org/podcasts", section: "Podcasts", maxResults: 5, waits: { page: 3000, type: 2000, search: 5000 } };

function genPython(cfg, recorder) { const ts = new Date().toISOString();
  return `"""Auto-generated – NPR Podcast Listing. Generated: ${ts}"""
...
def run(playwright: Playwright, section: str = "${cfg.section}", max_results: int = ${cfg.maxResults}) -> list:
    ...
    port = get_free_port(); profile_dir = get_temp_profile_dir("npr_org")
    ...
    try:
        ...
        for sel in ["button#onetrust-accept-btn-handler", "button:has-text('Accept')", "button:has-text('Close')", "button:has-text('No Thanks')"]:
            try:
                btn = page.locator(sel).first
                if btn.is_visible(timeout=1500): btn.evaluate("el => el.click()"); page.wait_for_timeout(500)
            except Exception: pass
        cards = page.locator('article, div[class*="podcast"], li[class*="podcast"], div[class*="item"]')
        count = cards.count()
        for i in range(min(count, max_results)):
            card = cards.nth(i); name = desc = "N/A"
            try: name = card.locator('h2, h3, h4, a[class*="title"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: desc = card.locator('p, [class*="description"], [class*="teaser"]').first.inner_text(timeout=2000).strip()[:200]
            except Exception: pass
            if name != "N/A": results.append({"name": name, "description": desc}); print(f"  {len(results)}. {name} — {desc[:80]}")
...
async function main() { const recorder = new PlaywrightRecorder(); const llmClient = setupLLMClient("hybrid"); let stagehand;
  try { stagehand = new Stagehand({ env: "LOCAL", verbose: 0, llmClient, localBrowserLaunchOptions: { userDataDir: path.join(os.homedir(), "AppData", "Local", "Google", "Chrome", "User Data", "Default"), headless: false, viewport: { width: 1920, height: 1080 }, args: ["--disable-blink-features=AutomationControlled", "--disable-infobars", "--disable-extensions", "--start-maximized"] } });
    ...
    const listings = await stagehand.extract(`Extract up to ${CFG.maxResults} podcasts with name and short description.`, z.object({ podcasts: z.array(z.object({ name: z.string(), description: z.string() })) }));
    recorder.record("extract", { instruction: "Extract podcasts", results: listings });
    fs.writeFileSync(path.join(__dirname, "npr_podcasts.py"), genPython(CFG, recorder), "utf-8");
