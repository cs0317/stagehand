...
const CFG = { url: "https://www.instacart.com", query: "organic milk", maxResults: 5, waits: { page: 3000, type: 2000, search: 5000 } };
function genPython(cfg, recorder) { const ts = new Date().toISOString();
  return `"""
Auto-generated – Instacart Product Search
Query: ${cfg.query}  Generated on: ${ts}
"""
...
def run(playwright: Playwright, query: str = "${cfg.query}", max_results: int = ${cfg.maxResults}) -> list:
    print(f"  Query: {query}\\n")
    port = get_free_port(); profile_dir = get_temp_profile_dir("instacart_com")
    ...
    try:
        ...
        search_input = page.locator('input[aria-label*="search" i], input[placeholder*="search" i], input[type="search"], input[role="combobox"]').first
        ...
        cards = page.locator('[data-testid="product-card"], div[class*="product-card"], li[class*="product"]')
        count = cards.count()
        for i in range(min(count, max_results)):
            card = cards.nth(i)
            try:
                name = price = store = "N/A"
                try: name = card.locator('[class*="name"], h3, h4, [data-testid="product-name"]').first.inner_text(timeout=2000).strip()
                except Exception: pass
                try: price = card.locator('[class*="price"], span:has-text("$")').first.inner_text(timeout=2000).strip()
                except Exception: pass
                try: store = card.locator('[class*="store"], [class*="retailer"]').first.inner_text(timeout=2000).strip()
                except Exception: pass
                if name != "N/A": results.append({"name": name, "price": price, "store": store}); print(f"  {len(results)}. {name} | {price} | {store}")
            except Exception as e: print(f"  Error: {e}")
        print(f"\\nFound {len(results)} products for '{query}':")
        for i, r in enumerate(results, 1): print(f"  {i}. {r['name']} — {r['price']} ({r['store']})")
...
async function main() { const recorder = new PlaywrightRecorder(); const llmClient = setupLLMClient("hybrid"); let stagehand;
  try { stagehand = new Stagehand({ env: "LOCAL", verbose: 0, llmClient, localBrowserLaunchOptions: { userDataDir: path.join(os.homedir(), "AppData", "Local", "Google", "Chrome", "User Data", "Default"), headless: false, viewport: { width: 1920, height: 1080 }, args: ["--disable-blink-features=AutomationControlled", "--disable-infobars", "--disable-extensions", "--start-maximized"] } });
    ...
    await observeAndAct(stagehand, page, recorder, "Click search", "Search"); await stagehand.act(`Type '${CFG.query}' and press Enter`); await page.waitForTimeout(CFG.waits.search);
    const { z } = require("zod/v3");
    const listings = await stagehand.extract(`Extract up to ${CFG.maxResults} products with name, price, store.`, z.object({ products: z.array(z.object({ name: z.string(), price: z.string(), store: z.string() })) }));
    fs.writeFileSync(path.join(__dirname, "instacart_search.py"), genPython(CFG, recorder), "utf-8");
