 ...
 *
 * Extracts latest cycling news articles:
    ...
    js_code = r${'`'}""(max) => {
        ...
        for (let i = 0; i < cards.length && articles.length < max; i++) {
            ...
            if (byIdx >= 0 && byIdx + 1 < lines.length) {
                ...
                // Strip leading category label like "NEWS " or "HOW TO WATCH "
...
(async () => {
  ...
  try {
    const url = "https://www.cyclingnews.com/news/";
    console.log(`Navigating to ${url}`);
    await page.goto(url, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(CFG.waits.page);

    const result = await stagehand.extract({
      instruction: `Extract the first ${CFG.maxArticles} news articles. For each get: title, author, publication date, and summary.`,
      schema: {
        type: "object",
        properties: {
          articles: {
    ...
    fs.writeFileSync(path.join(__dirname, "cyclingnews_headlines.py"), pyCode);
    console.log("\nSaved cyclingnews_headlines.py");
  } finally {
