 ...
 *
 * Extracts current space weather data:
 * solar wind speed, Kp index, Bz, density, flare activity, sunspot count.
 */
...
def spaceweather_conditions(page: Page) -> SpaceWeatherResult:
    ...
    if m: result.solar_wind_speed = m.group(1) + " km/sec"

    m = re.search(r'Kp(\\d[\\+\\-]?)\\n', body)
    if m: result.kp_index = "Kp" + m.group(1)

    m = re.search(r'Bz:\\s*([\\-\\d.]+)\\s*nT\\s*(North|South)?', body)
    if m: result.bz_value = m.group(1) + " nT" + (" " + m.group(2) if m.group(2) else "")

    m = re.search(r'Density:\\s*([\\d.]+)\\s*p/cm3', body)
    if m: result.density = m.group(1) + " p/cm3"

    m = re.search(r'Bt:\\s*([\\d.]+)\\s*nT', body)
    if m: result.bt_value = m.group(1) + " nT"

    m = re.search(r'Last X-flare\\s+(\\S+)\\s+(X[\\d.]+)', body)
    if m: result.last_x_flare = f"{m.group(2)} on {m.group(1)}"

    m = re.search(r'Last geomagnetic storm\\s+(\\S+)\\s+(Kp\\S+)\\s*\\((\\w+)\\)', body)
    if m: result.last_geomagnetic_storm = f"{m.group(2)} ({m.group(3)}) on {m.group(1)}"

    m = re.search(r'Monthly mean Sunspot Number\\n(\\w+\\s+\\d{4})\\s+([\\d.]+)', body)
    if m: result.sunspot_number = f"{m.group(2)} ({m.group(1)})"
...
(async () => {
  ...
  try {
    const url = "https://www.spaceweatherlive.com/";
    console.log(\`Navigating to \${url}\`);
    await page.goto(url, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(CFG.waits.page);

    const result = await stagehand.extract({
      instruction: "Extract current space weather conditions: solar wind speed, Kp index, Bz value, density, Bt, last X-flare, last geomagnetic storm, and sunspot number.",
      schema: {
        type: "object",
        properties: {
          solar_wind_speed: { type: "string" },
          kp_index: { type: "string" },
          bz_value: { type: "string" },
          density: { type: "string" },
          bt_value: { type: "string" },
          last_x_flare: { type: "string" },
          last_geomagnetic_storm: { type: "string" },
          sunspot_number: { type: "string" },
        },
      },
    });

    console.log("\\nExtracted space weather data:");
    ...
    fs.writeFileSync(path.join(__dirname, "spaceweather_conditions.py"), pyCode);
    console.log("\\nSaved spaceweather_conditions.py");
  } finally {
