...
def metmuseum_search(page: Page, request: ArtRequest) -> ArtResult:
    ...
    url = f"https://www.metmuseum.org/search-results#!/search?q={request.query.replace(' ', '%20')}"
    print(f"Loading {url}...")
    checkpoint("Navigate to Met Museum search")
    ...
    cards = page.locator("[class*='card'], [class*='result'] a, .search-results__item").all()
    for card in cards[:request.max_results]:
        try:
            ...
            for line in lines:
                ...
                if dm and not date:
                    date = line
                if any(kw in line.lower() for kw in ["oil", "canvas", "watercolor", "bronze", "marble", "paint"]):
                    medium = line
    ...
    if not artworks:
        ...
        for line in lines:
            if any(kw in line.lower() for kw in ["impressi", "monet", "renoir", "degas", "painting"]):
...
(async () => {
  ...
  try {
    const searchUrl = `https://www.metmuseum.org/search-results#!/search?q=${CFG.query}`;
    console.log(`\n🌐 Loading: ${searchUrl}...`);
    await page.goto(searchUrl, { waitUntil: "domcontentloaded", timeout: 30000 });
    await page.waitForTimeout(CFG.waits.page);
    recorder.record("goto", { url: searchUrl, description: "Navigate to Met Museum search" });

    const artworks = await stagehand.extract(
      `extract up to ${CFG.maxResults} artworks with title, artist name, date, medium, and image URL`
    );
    ...
    fs.writeFileSync(path.join(dir, "metmuseum_search.py"), pyCode);
