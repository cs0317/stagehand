/**
 * Mercari – Secondhand Item Search
 *
 * Search for "Nintendo Switch", extract up to 5 listings with title, price,
 * condition, and seller name.
 */
...
const CFG = { query: "Nintendo Switch", maxItems: 5 };
...
(async () => {
  ...
  try {
    const url = `https://www.mercari.com/search/?keyword=${encodeURIComponent(CFG.query)}`;
    ...
    console.log("\n📐 DOM Exploration:");
    for (const sel of ['div[data-testid="SearchResults"]', 'a[data-testid*="item"]', 'div[class*="ItemCard"]', 'a[href*="/item/"]']) {
    ...
    const data = await stagehand.extract(
      `Extract the first ${CFG.maxItems} item listings. For each get: item title, price, condition, and seller name.`,
      z.object({
        listings: z.array(z.object({
          title: z.string(), price: z.string(), condition: z.string().optional(), seller: z.string().optional(),
        })),
      }),
    );
    console.log("\n📊 Extracted data:");
