...
(async () => {
  ...
  try {
    ...
    const url = `https://www.rover.com/search/?location=${location}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} pet sitter results. For each get the sitter name, rating, number of reviews, price per night, services offered, distance, and response time.`
    );
    recorder.record("extract", "sitter listings", JSON.stringify(data));
    console.log("Extracted:", JSON.stringify(data, null, 2));

    const outDir = __dirname;
    fs.writeFileSync(path.join(outDir, "recorded_actions.json"), JSON.stringify(recorder.actions, null, 2));
    fs.writeFileSync(path.join(outDir, "rover_search.py"), genPython(CFG, recorder));
    console.log("Saved rover_search.py");
  } finally {
