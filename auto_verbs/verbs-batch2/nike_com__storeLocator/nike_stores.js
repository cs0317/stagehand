...
const CFG = { url: "https://www.nike.com/retail", location: "Los Angeles, CA", maxResults: 3, waits: { page: 3000, type: 2000, search: 5000 } };

function genPython(cfg, recorder) { const ts = new Date().toISOString();
  return `"""Auto-generated – Nike Store Locator. Location: ${cfg.location}. Generated: ${ts}"""
...
def run(playwright: Playwright, location: str = "${cfg.location}", max_results: int = ${cfg.maxResults}) -> list:
    ...
    port = get_free_port(); profile_dir = get_temp_profile_dir("nike_com__storeLocator")
    ...
    try:
        ...
        si = page.locator('input[id*="search"], input[placeholder*="city" i], input[placeholder*="location" i], input[aria-label*="search" i], input[type="search"]').first
        ...
        cards = page.locator('[data-testid*="store"], [class*="store-card"], [class*="StoreCard"], li[class*="result"]')
        count = cards.count()
        for i in range(min(count, max_results)):
            card = cards.nth(i); name = address = hours = "N/A"
            try: name = card.locator('h2, h3, [class*="store-name"], [class*="title"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: address = card.locator('address, [class*="address"], p[class*="location"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: hours = card.locator('[class*="hours"], span:has-text("Open"), span:has-text("Closed")').first.inner_text(timeout=2000).strip()
            except Exception: pass
            if name != "N/A": results.append({"name": name, "address": address, "hours": hours}); print(f"  {len(results)}. {name} | {address} | {hours}")
        print(f"\\nFound {len(results)} stores:")
        for i, r in enumerate(results, 1): print(f"  {i}. {r['name']}\\n     {r['address']}\\n     {r['hours']}")
...
async function main() { const recorder = new PlaywrightRecorder(); const llmClient = setupLLMClient("hybrid"); let stagehand;
  try { stagehand = new Stagehand({ env: "LOCAL", verbose: 0, llmClient, localBrowserLaunchOptions: { userDataDir: path.join(os.homedir(), "AppData", "Local", "Google", "Chrome", "User Data", "Default"), headless: false, viewport: { width: 1920, height: 1080 }, args: ["--disable-blink-features=AutomationControlled", "--disable-infobars", "--disable-extensions", "--start-maximized"] } });
    ...
    const listings = await stagehand.extract(`Extract up to ${CFG.maxResults} Nike stores with name, address, and hours.`, z.object({ stores: z.array(z.object({ name: z.string(), address: z.string(), hours: z.string() })) }));
    recorder.record("extract", { instruction: "Extract stores", results: listings });
    fs.writeFileSync(path.join(__dirname, "nike_stores.py"), genPython(CFG, recorder), "utf-8");
