...
const CFG = { url: "https://www.tripadvisor.com", destination: "San Diego", maxResults: 5, waits: { page: 3000, type: 2000, search: 5000 } };

function genPython(cfg, recorder) { const ts = new Date().toISOString();
  return `"""Auto-generated – TripAdvisor Hotel Search. Destination: ${cfg.destination}. Generated: ${ts}"""
import re, os, sys, shutil
from playwright.sync_api import Playwright, sync_playwright
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))
from cdp_utils import get_free_port, get_temp_profile_dir, launch_chrome, wait_for_cdp_ws

def run(playwright: Playwright, destination: str = "${cfg.destination}", max_results: int = ${cfg.maxResults}) -> list:
    print(f"  Destination: {destination}\\n")
    port = get_free_port(); profile_dir = get_temp_profile_dir("tripadvisor_com")
    ...
    try:
        ...
        si = page.locator('input[placeholder*="Where" i], input[aria-label*="search" i], input[type="search"]').first
        si.evaluate("el => el.click()"); page.wait_for_timeout(500)
        page.keyboard.press("Control+a"); si.type(f"{destination} hotels", delay=50); page.wait_for_timeout(1000); page.keyboard.press("Enter")
        page.wait_for_timeout(3000); page.wait_for_load_state("domcontentloaded"); page.wait_for_timeout(2000)
        cards = page.locator('[data-testid*="property-card"], div[class*="listing_title"], div[class*="property-card"], div[class*="listItem"]')
        count = cards.count()
        for i in range(min(count, max_results)):
            card = cards.nth(i); name = price = rating = "N/A"
            try: name = card.locator('a[class*="property_title"], [data-testid*="title"], h3, a[class*="title"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: price = card.locator('[data-testid*="price"], [class*="price"], span:has-text("$")').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: rating = card.locator('[class*="bubble"], [aria-label*="bubble"], svg[class*="rating"]').first.get_attribute("aria-label", timeout=2000) or "N/A"
            except Exception: pass
...
async function main() { const recorder = new PlaywrightRecorder(); const llmClient = setupLLMClient("hybrid"); let stagehand;
  try { stagehand = new Stagehand({ env: "LOCAL", verbose: 0, llmClient, localBrowserLaunchOptions: { userDataDir: path.join(os.homedir(), "AppData", "Local", "Google", "Chrome", "User Data", "Default"), headless: false, viewport: { width: 1920, height: 1080 }, args: ["--disable-blink-features=AutomationControlled", "--disable-infobars", "--disable-extensions", "--start-maximized"] } });
    ...
    await stagehand.act(`Type '${CFG.destination} hotels' and press Enter`); await page.waitForTimeout(CFG.waits.search);
    const { z } = require("zod/v3");
    const listings = await stagehand.extract(`Extract up to ${CFG.maxResults} hotels with name, price per night, and traveler rating.`, z.object({ hotels: z.array(z.object({ name: z.string(), price: z.string(), rating: z.string() })) }));
    recorder.record("extract", { instruction: "Extract hotels", results: listings });
    fs.writeFileSync(path.join(__dirname, "tripadvisor_hotels.py"), genPython(CFG, recorder), "utf-8");
