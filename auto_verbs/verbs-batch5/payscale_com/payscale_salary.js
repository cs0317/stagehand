...
/**
 * payscale.com – Salary Data for a Job Title
 *
 * Navigates to the PayScale salary page for a given job title
 * and extracts average salary, salary range, and experience-level pay.
 */

// ── Configuration ────────────────────────────────────────────────────────────
const CFG = {
  jobTitle: "Data Scientist",
  jobSlug: "Data_Scientist",
  waits: { page: 5000 },
};
...
class PayScaleRequest:
    job_title: str = "${cfg.jobTitle}"
    job_slug: str = "${cfg.jobSlug}"
...
(async () => {
  ...
  try {
    const url = `https://www.payscale.com/research/US/Job=${CFG.jobSlug}/Salary`;
    console.log(`\n🌐 Navigating to ${url}...`);
    await page.goto(url, { waitUntil: "domcontentloaded", timeout: 30000 });
    await page.waitForTimeout(CFG.waits.page);
    recorder.record("goto", { url, description: "Navigate to PayScale salary page" });
    ...
    recorder.record("extract", {
      instruction: "Extract salary data",
      description: `Extracted salary data for ${CFG.jobTitle}`,
      results: data,
    });

    console.log("\n" + "=".repeat(60));
    console.log(`PayScale Salary Data: ${CFG.jobTitle}`);
    console.log("=".repeat(60));
    console.log(`\n   Average Salary: ${data.avgSalary}`);
    console.log(`   Salary Range: ${data.rangeLow} - ${data.rangeHigh}`);
    console.log(`   Bonus Range: ${data.bonus}`);
    console.log(`   Total Pay Range: ${data.totalPay}`);
    console.log(`   Based on: ${data.numProfiles} salary profiles`);

    if (data.levels.length > 0) {
      console.log("\n   Pay by Experience Level:");
      data.levels.forEach(lvl => {
        console.log(`     ${lvl.level}: ${lvl.direction}${lvl.percent}`);
      });
    }
    ...
    fs.writeFileSync(path.join(dir, "payscale_salary.py"), pyCode);
