...
const CFG = { searchQuery: "how solar panels work", maxResults: 5 };
...
(async () => {
  ...
  try {
    const url = `https://www.howstuffworks.com/search.php?terms=${encodeURIComponent(CFG.searchQuery)}`;
    ...
    const items = await page.evaluate((max) => {
      const results = [];
      const cards = document.querySelectorAll('article, [class*="search-result"], [class*="content-card"], [class*="result"], li[class*="result"]');
      for (const card of cards) {
        ...
        let summary = '';
        const sumEl = card.querySelector('p, [class*="description"], [class*="excerpt"]');
        if (sumEl) summary = sumEl.textContent.trim().substring(0, 200);

        results.push({ title, author, category, summary });
      }
      return results;
    }, CFG.maxResults);
