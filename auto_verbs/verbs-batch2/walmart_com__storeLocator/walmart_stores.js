...
const CFG = { url: "https://www.walmart.com/store/finder", location: "Austin, TX", maxResults: 3, waits: { page: 3000, type: 2000, search: 5000 } };

function genPython(cfg, recorder) { const ts = new Date().toISOString();
  return `"""Auto-generated – Walmart Store Finder. Location: ${cfg.location}. Generated: ${ts}"""
...
def run(playwright: Playwright, location: str = "${cfg.location}", max_results: int = ${cfg.maxResults}) -> list:
    ...
    port = get_free_port(); profile_dir = get_temp_profile_dir("walmart_com__storeLocator")
    ...
    try:
        ...
        cards = page.locator('[data-testid*="store"], div[class*="store-card"], li[class*="store"]')
        count = cards.count()
        for i in range(min(count, max_results)):
            card = cards.nth(i); address = hours = phone = "N/A"
            try: address = card.locator('address, [class*="address"], p[class*="location"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: hours = card.locator('[class*="hours"], [class*="time"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: phone = card.locator('a[href^="tel:"], [class*="phone"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            if address != "N/A": results.append({"address": address, "hours": hours, "phone": phone}); print(f"  {len(results)}. {address} | {hours} | {phone}")
...
async function main() { const recorder = new PlaywrightRecorder(); const llmClient = setupLLMClient("hybrid"); let stagehand;
  try { stagehand = new Stagehand({ env: "LOCAL", verbose: 0, llmClient, localBrowserLaunchOptions: { userDataDir: path.join(os.homedir(), "AppData", "Local", "Google", "Chrome", "User Data", "Default"), headless: false, viewport: { width: 1920, height: 1080 }, args: ["--disable-blink-features=AutomationControlled", "--disable-infobars", "--disable-extensions", "--start-maximized"] } });
    ...
    await observeAndAct(stagehand, page, recorder, "Click location search input", "Search");
    await stagehand.act(`Type '${CFG.location}' and press Enter`); await page.waitForTimeout(CFG.waits.search);
    const { z } = require("zod/v3");
    const listings = await stagehand.extract(`Extract up to ${CFG.maxResults} stores with address, hours, and phone number.`, z.object({ stores: z.array(z.object({ address: z.string(), hours: z.string(), phone: z.string() })) }));
    recorder.record("extract", { instruction: "Extract stores", results: listings });
    fs.writeFileSync(path.join(__dirname, "walmart_stores.py"), genPython(CFG, recorder), "utf-8");
