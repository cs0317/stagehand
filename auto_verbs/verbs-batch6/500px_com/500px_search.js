 ...
 * 500px – Photo Search
 *
 * Searches 500px.com for photos matching a query and extracts
 * photo title, photographer name, number of likes, and photo URL.
 */
...
(async () => {
  ...
  try {
    const searchUrl = `https://500px.com/search?q=${encodeURIComponent(CFG.query)}&type=photos`;
    console.log(`\n🌐 Searching: ${searchUrl}...`);
    await page.goto(searchUrl, { waitUntil: "domcontentloaded", timeout: 30000 });
    await page.waitForTimeout(CFG.waits.page);
    recorder.record("goto", { url: searchUrl, description: "Search 500px photos" });

    // Extract photos using page.evaluate
    const photos = await page.evaluate((maxResults) => {
      const results = [];
      const links = document.querySelectorAll('a[href*="/photo/"]');
      const seen = new Set();
      for (const link of links) {
    ...
    recorder.record("extract", {
      instruction: "Extract photo search results",
      description: `Extracted ${photos.length} photos`,
      results: photos,
    });

    console.log("\n" + "=".repeat(60));
    console.log(`500px Photos: ${CFG.query}`);
    ...
    fs.writeFileSync(path.join(dir, "500px_search.py"), pyCode);
