 ...
 *
 * Searches allaboutbirds.org for a bird species and extracts details:
 * common name, scientific name, habitat, diet, conservation status, etc.
 */
...
(async () => {
  ...
  try {
    const slug = CFG.birdName.replace(/ /g, "_").replace(/'/g, "");
    const url = `https://www.allaboutbirds.org/guide/${slug}/overview`;
    console.log(`\\n🌐 Navigating to ${url}...`);
    await page.goto(url, { waitUntil: "domcontentloaded", timeout: 30000 });
    await page.waitForTimeout(CFG.waits.page);
    recorder.record("goto", { url, description: "Navigate to bird guide page" });

    // Extract bird info using Stagehand
    console.log("\\n🔍 Extracting bird information...");
    const birdData = await stagehand.extract(
      "Extract the bird's common name, scientific name, habitat, food/diet, and conservation status from this page"
    );
    console.log("\\n📊 Bird data:", JSON.stringify(birdData, null, 2));
    recorder.record("extract", {
      instruction: "Extract bird species info",
      description: "Extracted bird details",
      results: birdData,
    });
    ...
    fs.writeFileSync(path.join(dir, "allaboutbirds_lookup.py"), pyCode);
    console.log("🐍 Saved allaboutbirds_lookup.py");
  } catch (err) {
