...
(async () => {
  ...
  try {
    const wordUrl = `https://www.merriam-webster.com/dictionary/${CFG.word}`;
    console.log(`\n🌐 Loading: ${wordUrl}...`);
    await page.goto(wordUrl, { waitUntil: "domcontentloaded", timeout: 30000 });
    await page.waitForTimeout(CFG.waits.page);
    recorder.record("goto", { url: wordUrl, description: "Navigate to Merriam-Webster" });

    const wordData = await stagehand.extract(
      "extract the word, pronunciation, part of speech, definition, example sentence, and first known use date"
    );
    console.log("\n📊 Word:", JSON.stringify(wordData, null, 2));
    recorder.record("extract", { instruction: "Extract word data", results: wordData });
    ...
    fs.writeFileSync(path.join(dir, "mw_lookup.py"), pyCode);
