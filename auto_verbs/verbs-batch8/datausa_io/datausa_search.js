...
(async () => {
  ...
  try {
    ...
    const url = `https://datausa.io/search?q=${query}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} location search results. For each get the location name, population, median income, poverty rate, median age, and employment rate.`
    );
    recorder.record("extract", "location data listings", JSON.stringify(data));
    console.log("Extracted:", JSON.stringify(data, null, 2));

    const outDir = __dirname;
    fs.writeFileSync(path.join(outDir, "recorded_actions.json"), JSON.stringify(recorder.actions, null, 2));
    fs.writeFileSync(path.join(outDir, "datausa_search.py"), genPython(CFG, recorder));
    console.log("Saved datausa_search.py");
  } finally {
