...
(async () => {
  ...
  try {
    const url = `https://data.census.gov/all?q=${encodeURIComponent(CFG.query)}`;
    console.log(`\n🌐 Searching: ${url}...`);
    await page.goto(url, { waitUntil: "domcontentloaded", timeout: 30000 });
    await page.waitForTimeout(8000);
    recorder.record("goto", { url, description: "Search Census data" });

    const tablesData = await stagehand.extract(
      "extract up to 5 data tables with table title, geographic area, year, and key population figure"
    );
    console.log("\n📊 Tables:", JSON.stringify(tablesData, null, 2));
    recorder.record("extract", { instruction: "Extract census tables", results: tablesData });
    ...
    fs.writeFileSync(path.join(dir, "census_search.py"), pyCode);
