...
(async () => {
  ...
  try {
    const url = `https://www.drugs.com/${CFG.drug}.html`;
    console.log(`\n🌐 Loading: ${url}...`);
    await page.goto(url, { waitUntil: "domcontentloaded", timeout: 30000 });
    await page.waitForTimeout(CFG.waits.page);
    recorder.record("goto", { url, description: "Navigate to drug page" });

    const drugData = await stagehand.extract(
      "extract the drug name, generic name, drug class, common uses, common side effects, and dosage information"
    );
    console.log("\n📊 Drug:", JSON.stringify(drugData, null, 2));
    recorder.record("extract", { instruction: "Extract drug info", results: drugData });
    ...
    fs.writeFileSync(path.join(dir, "drugs_search.py"), pyCode);
