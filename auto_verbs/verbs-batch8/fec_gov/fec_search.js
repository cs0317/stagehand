...
(async () => {
  ...
  try {
    ...
    const url = `https://www.fec.gov/data/candidates/?search=${query}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} candidate results. For each get the candidate name, party, office, state, total receipts, total disbursements, cash on hand, and election year.`
    );
    recorder.record("extract", "candidate data", JSON.stringify(data));
    console.log("Extracted:", JSON.stringify(data, null, 2));

    const outDir = __dirname;
    fs.writeFileSync(path.join(outDir, "recorded_actions.json"), JSON.stringify(recorder.actions, null, 2));
    fs.writeFileSync(path.join(outDir, "fec_search.py"), genPython(CFG, recorder));
    console.log("Saved fec_search.py");
  } finally {
