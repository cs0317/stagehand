...
(async () => {
  ...
  try {
    const url = `https://www.google.com/travel/hotels/${CFG.destination}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} hotel listings. For each get the hotel name, price per night, rating, number of reviews, amenities, and distance from center.`
    );
    ...
    fs.writeFileSync(path.join(outDir, "google_hotels_search.py"), genPython(CFG, recorder));
    console.log("Saved google_hotels_search.py");
  } finally {
