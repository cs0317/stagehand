...
def eater_search(page: Page, request: EaterSearchRequest) -> EaterSearchResult:
    ...
    js_code = """(max) => {
        const cards = document.querySelectorAll('article, [class*="SearchResult"], [class*="search-result"], [class*="Card"], .c-entry-box--compact');
        const items = [];
...
(async () => {
  ...
  try {
    ...
    const url = `https://www.eater.com/search?q=${query}`;
    ...
    fs.writeFileSync(path.join(outDir, "eater_search.py"), genPython(CFG, recorder));
    console.log("Saved eater_search.py");
  } finally {
