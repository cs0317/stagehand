...
(async () => {
  ...
  try {
    ...
    const url = `https://wallethub.com/search?q=${query}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} search results. For each get the title, category, publish date, summary, and URL.`
    );
    ...
    fs.writeFileSync(path.join(outDir, "wallethub_search.py"), genPython(CFG, recorder));
    console.log("Saved wallethub_search.py");
  } finally {
