 ...
 *
 * Loads the TripAdvisor "Things to Do" page for a city and extracts
 * top attractions with name, rating, reviews, and category.
 */

// ── Configuration ────────────────────────────────────────────────────────────
const CFG = {
  city: "Barcelona",
  citySlug: "Barcelona_Catalonia",
  geoId: "g187497",
  maxAttractions: 5,
  waits: { page: 10000 },
};
...
class TripAdvisorRequest:
    city: str = "${cfg.city}"
    city_slug: str = "${cfg.citySlug}"
    geo_id: str = "${cfg.geoId}"
    max_attractions: int = ${cfg.maxAttractions}
...
(async () => {
  ...
  try {
    const url = `https://www.tripadvisor.com/Attractions-${CFG.geoId}-Activities-${CFG.citySlug}.html`;
    console.log(`\n🌐 Loading: ${url}...`);
    await page.goto(url, { waitUntil: "domcontentloaded", timeout: 30000 });
    await page.waitForTimeout(CFG.waits.page);
    recorder.record("goto", { url, description: "Load TripAdvisor things to do" });

    // Check for CAPTCHA
    const bodyLen = await page.evaluate(() => document.body.innerText.length);
    if (bodyLen === 0) {
      console.log("⚠️ CAPTCHA detected, reloading...");
    ...
    // Extract from body text via regex
    const attractions = await page.evaluate((maxAttractions) => {
      const text = document.body.innerText;
      const idx = text.indexOf("\n1. ");
      if (idx === -1) return [];
      const section = text.substring(idx);
    ...
    recorder.record("extract", {
      instruction: "Extract top attractions",
      description: `Extracted ${attractions.length} attractions`,
      results: attractions,
    });

    console.log("\n" + "=".repeat(60));
    console.log(`TripAdvisor: Things to Do in ${CFG.city}`);
    console.log("=".repeat(60));
    attractions.forEach((a, i) => {
      console.log(`\n  ${i + 1}. ${a.name}`);
      console.log(`     Rating: ${a.rating} (${a.num_reviews} reviews)`);
    ...
    fs.writeFileSync(path.join(dir, "tripadvisor_thingsToDo.py"), pyCode);
