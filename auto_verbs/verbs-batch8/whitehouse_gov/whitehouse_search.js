...
(async () => {
  ...
  try {
    ...
    const url = `https://www.whitehouse.gov/?s=${query}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} briefing or statement results. For each get the title, publish date, category, summary, and URL.`
    );
    recorder.record("extract", "briefing listings", JSON.stringify(data));
    console.log("Extracted:", JSON.stringify(data, null, 2));

    const outDir = __dirname;
    fs.writeFileSync(path.join(outDir, "recorded_actions.json"), JSON.stringify(recorder.actions, null, 2));
    fs.writeFileSync(path.join(outDir, "whitehouse_search.py"), genPython(CFG, recorder));
    console.log("Saved whitehouse_search.py");
  } finally {
