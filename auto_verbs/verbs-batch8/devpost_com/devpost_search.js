...
(async () => {
  ...
  try {
    ...
    const url = `https://devpost.com/software/search?query=${query}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} hackathon project results. For each get the project name, tagline, number of likes, number of comments, technologies built with, and hackathon name.`
    );
    recorder.record("extract", "project listings", JSON.stringify(data));
    console.log("Extracted:", JSON.stringify(data, null, 2));

    const outDir = __dirname;
    fs.writeFileSync(path.join(outDir, "recorded_actions.json"), JSON.stringify(recorder.actions, null, 2));
    fs.writeFileSync(path.join(outDir, "devpost_search.py"), genPython(CFG, recorder));
    console.log("Saved devpost_search.py");
  } finally {
