 ...
 *
 * Searches iFixit for repair guides, then visits each guide
 * to extract difficulty, estimated time, and step count.
 */

// ── Configuration ────────────────────────────────────────────────────────────
const CFG = {
  url: "https://www.ifixit.com/Search",
  searchQuery: "iPhone 15 battery replacement",
  maxResults: 3,
  waits: { page: 5000, guide: 3000 },
};
...
(async () => {
  ...
  try {
    ...
    recorder.record("goto", { url, description: `Search iFixit for "${CFG.searchQuery}"` });

    const guideLinks = await page.evaluate((maxResults) => {
      const links = document.querySelectorAll('a[href*="/Guide/"]');
      const seen = new Set();
      const results = [];
      for (const a of links) {
        ...
        seen.add(href);
        const title = a.innerText.split("\n")[0].trim();
        if (title && title.length > 5) results.push({ title, url: href });
      }
      return results;
    }, CFG.maxResults);
    ...
    for (const g of guideLinks) {
      console.log(`   📖 Visiting: ${g.title}...`);
      await page.goto(g.url, { waitUntil: "domcontentloaded", timeout: 30000 });
      await page.waitForTimeout(CFG.waits.guide);
      ...
      const meta = await page.evaluate(() => {
        ...
        const lastStep = stepMatches ? stepMatches[stepMatches.length - 1] : null;
        const numSteps = lastStep ? parseInt(lastStep.replace("Step ", "")) : 0;
        return { difficulty: diffMatch ? diffMatch[1] : "N/A", time: timeMatch ? timeMatch[0] : "N/A", numSteps };
      });

      results.push({ ...g, ...meta });
      console.log(`      Difficulty: ${meta.difficulty} | Time: ${meta.time} | Steps: ${meta.numSteps}`);
    }

    recorder.record("extract", {
      instruction: "Extract iFixit repair guides",
      description: `Extracted ${results.length} guides with metadata`,
      results,
    });
    ...
    results.forEach((r, i) => {
      ...
      console.log(`      Difficulty: ${r.difficulty} | Time: ${r.time} | Steps: ${r.numSteps}`);
    ...
    fs.writeFileSync(path.join(dir, "ifixit_search.py"), genPython(CFG, recorder));
