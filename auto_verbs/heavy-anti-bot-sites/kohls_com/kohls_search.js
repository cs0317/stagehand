...
const CFG = { url: "https://www.kohls.com", query: "kitchen appliances", maxResults: 5, waits: { page: 3000, type: 2000, search: 5000 } };
function genPython(cfg, recorder) { const ts = new Date().toISOString();
  return `"""Auto-generated – Kohl's Product Search. Query: ${cfg.query}. Generated on: ${ts}"""
...
def run(playwright: Playwright, query: str = "${cfg.query}", max_results: int = ${cfg.maxResults}) -> list:
    print(f"  Query: {query}\\n")
    port = get_free_port(); profile_dir = get_temp_profile_dir("kohls_com")
    ...
    try:
        ...
        si = page.locator('input[id="search-input"], input[name="search"], input[aria-label*="search" i], input[type="search"]').first
        ...
        for i in range(min(count, max_results)):
            ...
            try: name = card.locator('[class*="title"], h3, h4, [class*="product-name"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: price = card.locator('[class*="price"], span:has-text("$")').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: rating = card.locator('[class*="rating"], [class*="star"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
...
async function main() { const recorder = new PlaywrightRecorder(); const llmClient = setupLLMClient("hybrid"); let stagehand;
  try { stagehand = new Stagehand({ env: "LOCAL", verbose: 0, llmClient, localBrowserLaunchOptions: { userDataDir: path.join(os.homedir(), "AppData", "Local", "Google", "Chrome", "User Data", "Default"), headless: false, viewport: { width: 1920, height: 1080 }, args: ["--disable-blink-features=AutomationControlled", "--disable-infobars", "--disable-extensions", "--start-maximized"] } });
    ...
    const listings = await stagehand.extract(`Extract up to ${CFG.maxResults} products with name, price, rating.`, z.object({ products: z.array(z.object({ name: z.string(), price: z.string(), rating: z.string() })) }));
    fs.writeFileSync(path.join(__dirname, "kohls_search.py"), genPython(CFG, recorder), "utf-8");
