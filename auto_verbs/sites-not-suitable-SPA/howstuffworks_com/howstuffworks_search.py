...
# up to max_results articles with title, author, category, and summary.
def search_howstuffworks(page: Page, request: HowStuffWorksRequest) -> HowStuffWorksResult:
    ...
    js_code = """(max) => {
        ...
        for (const h4 of h4s) {
            ...
            let category = '';
            let summary = '';
            const pEl = card ? card.querySelector('p') : null;
            if (pEl) summary = pEl.textContent.trim().substring(0, 200);
    ...
    for i, item in enumerate(result.items, 1):
        print(f"\n  {i}. {item.title}")
        print(f"     Author: {item.author}  Category: {item.category}")
        if item.summary:
