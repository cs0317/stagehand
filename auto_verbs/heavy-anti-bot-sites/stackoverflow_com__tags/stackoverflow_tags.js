...
const CFG = { url: "https://stackoverflow.com/tags", section: "Tags", maxResults: 10, waits: { page: 3000, type: 2000, search: 5000 } };

function genPython(cfg, recorder) { const ts = new Date().toISOString();
  return `"""Auto-generated – Stack Overflow Tags Listing. Generated: ${ts}"""
...
def run(playwright: Playwright, section: str = "${cfg.section}", max_results: int = ${cfg.maxResults}) -> list:
    ...
    port = get_free_port(); profile_dir = get_temp_profile_dir("stackoverflow_com__tags")
    ...
    try:
        ...
        cards = page.locator('[id*="tags-browser"] div.d-flex, div[class*="tag-cell"], div[id*="tag"]')
        count = cards.count()
        for i in range(min(count, max_results)):
            card = cards.nth(i); name = qcount = desc = "N/A"
            try: name = card.locator('a[class*="post-tag"], a[href*="/questions/tagged/"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: qcount = card.locator('[class*="tag-counts"], span[class*="count"], div:has-text("question")').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: desc = card.locator('[class*="excerpt"], p, div[class*="description"]').first.inner_text(timeout=2000).strip()[:200]
            except Exception: pass
            if name != "N/A": results.append({"name": name, "question_count": qcount, "description": desc}); print(f"  {len(results)}. {name} | {qcount} | {desc[:60]}")
        print(f"\\nFound {len(results)} tags:")
        for i, r in enumerate(results, 1): print(f"  {i}. {r['name']} ({r['question_count']})\\n     {r['description'][:100]}")
...
async function main() { const recorder = new PlaywrightRecorder(); const llmClient = setupLLMClient("hybrid"); let stagehand;
  try { stagehand = new Stagehand({ env: "LOCAL", verbose: 0, llmClient, localBrowserLaunchOptions: { userDataDir: path.join(os.homedir(), "AppData", "Local", "Google", "Chrome", "User Data", "Default"), headless: false, viewport: { width: 1920, height: 1080 }, args: ["--disable-blink-features=AutomationControlled", "--disable-infobars", "--disable-extensions", "--start-maximized"] } });
    ...
    const listings = await stagehand.extract(`Extract the top ${CFG.maxResults} tags with name, question count, and description.`, z.object({ tags: z.array(z.object({ name: z.string(), questionCount: z.string(), description: z.string() })) }));
    recorder.record("extract", { instruction: "Extract tags", results: listings });
    fs.writeFileSync(path.join(__dirname, "stackoverflow_tags.py"), genPython(CFG, recorder), "utf-8");
