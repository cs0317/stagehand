 ...
 *
 * Looks up strength standards: beginner, novice, intermediate, advanced, elite.
 */
...
class StandardsRequest:
    exercise_slug: str = "${cfg.exerciseSlug}"
    bodyweight: int = ${cfg.bodyweight}
...
(async () => {
  ...
  try {
    const url = \`https://strengthlevel.com/strength-standards/\${CFG.exerciseSlug}/lb\`;
    console.log(\`Navigating to \${url}\`);
    await page.goto(url, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(CFG.waits.page);

    const result = await stagehand.extract({
      instruction: \`Extract the strength standards for a \${CFG.bodyweight} lb male: beginner, novice, intermediate, advanced, and elite weight levels.\`,
      schema: {
        type: "object",
        properties: {
    ...
    fs.writeFileSync(path.join(__dirname, "strengthlevel_standards.py"), pyCode);
    console.log("\\nSaved strengthlevel_standards.py");
  } finally {
