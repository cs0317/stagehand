...
(async () => {
  ...
  try {
    ...
    const url = `https://www.nytimes.com/wirecutter/search/?s=${query}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} product recommendation results. For each get the title, top pick, runner up, budget pick, publish date, and summary.`
    );
    recorder.record("extract", "product recommendation listings", JSON.stringify(data));
    console.log("Extracted:", JSON.stringify(data, null, 2));

    const outDir = __dirname;
    fs.writeFileSync(path.join(outDir, "recorded_actions.json"), JSON.stringify(recorder.actions, null, 2));
    fs.writeFileSync(path.join(outDir, "wirecutter_search.py"), genPython(CFG, recorder));
    console.log("Saved wirecutter_search.py");
  } finally {
