...
class BundlephobiaSearchRequest:
    package_name: str = "${cfg.packageName}"
...
(async () => {
  ...
  try {
    const url = `https://bundlephobia.com/package/${CFG.packageName}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the bundle size information for this npm package including package name, version, minified size, gzipped size, download times for 3G and 4G, number of dependencies, and whether it is tree-shakeable.`
    );
    recorder.record("extract", "bundle info", JSON.stringify(data));
    console.log("Extracted:", JSON.stringify(data, null, 2));

    const outDir = __dirname;
    fs.writeFileSync(path.join(outDir, "recorded_actions.json"), JSON.stringify(recorder.actions, null, 2));
    fs.writeFileSync(path.join(outDir, "bundlephobia_search.py"), genPython(CFG, recorder));
    console.log("Saved bundlephobia_search.py");
  } finally {
