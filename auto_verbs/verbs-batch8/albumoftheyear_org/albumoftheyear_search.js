...
Album of the Year – Browse highest-rated albums
...
(async () => {
  ...
  try {
    const url = `https://www.albumoftheyear.org/ratings/6-highest-rated/${CFG.year}/1`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} highest-rated albums. For each get the title, artist, aggregate critic score, number of reviews, user score, and genre.`
    );
    recorder.record("extract", "album listings", JSON.stringify(data));
    console.log("Extracted:", JSON.stringify(data, null, 2));

    const outDir = __dirname;
    fs.writeFileSync(path.join(outDir, "recorded_actions.json"), JSON.stringify(recorder.actions, null, 2));
    fs.writeFileSync(path.join(outDir, "albumoftheyear_search.py"), genPython(CFG, recorder));
    console.log("Saved albumoftheyear_search.py");
  } finally {
