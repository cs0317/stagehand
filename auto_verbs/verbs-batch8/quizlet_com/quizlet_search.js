...
(async () => {
  ...
  try {
    ...
    const url = `https://quizlet.com/search?query=${query}&type=sets`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} study set results. For each get the set title, creator, number of terms, number of learners, rating, and subject.`
    );
    recorder.record("extract", "study set listings", JSON.stringify(data));
    console.log("Extracted:", JSON.stringify(data, null, 2));

    const outDir = __dirname;
    fs.writeFileSync(path.join(outDir, "recorded_actions.json"), JSON.stringify(recorder.actions, null, 2));
    fs.writeFileSync(path.join(outDir, "quizlet_search.py"), genPython(CFG, recorder));
    console.log("Saved quizlet_search.py");
  } finally {
