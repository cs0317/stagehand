...
def search_therealreal(page: Page, request: TheRealRealRequest) -> TheRealRealResult:
    ...
    js_code = """(max) => {
        ...
        for (const a of links) {
            ...
            const designer = lines.length > 0 ? lines[0] : '';
            const name = lines.length > 1 ? lines.slice(1).join(' ') : '';
            if (!name && !designer) continue;
            results.push({ name: name || designer, designer, condition: '', retail_price: '', sale_price: '' });
        }
        return results;
    }"""
