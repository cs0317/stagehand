 ...
 *
 * Searches songsterr.com for guitar tabs:
 * song title, artist, variant.
 */

const CFG = {
  searchQuery: "Stairway to Heaven Led Zeppelin",
  maxResults: 5,
  waits: { page: 5000 },
};
...
(async () => {
  ...
  try {
    ...
    const url = `https://www.songsterr.com/?pattern=${query.replace(/ /g, "+")}`;
    console.log(`Navigating to ${url}`);
    await page.goto(url, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(CFG.waits.page);

    const result = await stagehand.extract({
      instruction: `Extract the first ${CFG.maxResults} tab search results. For each get: song title, artist, and tab variant/type.`,
      schema: {
        type: "object",
        properties: {
          tabs: {
    ...
    console.log(`\nExtracted ${result.tabs?.length || 0} tabs`);
    ...
    fs.writeFileSync(path.join(__dirname, "songsterr_search.py"), pyCode);
    console.log("\nSaved songsterr_search.py");
  } finally {
