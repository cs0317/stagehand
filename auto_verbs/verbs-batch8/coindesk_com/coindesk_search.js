...
def coindesk_search(page: Page, request: CoindeskSearchRequest) -> CoindeskSearchResult:
    ...
    js_code = """(max) => {
        const cards = document.querySelectorAll('article, [class*="SearchResult"], [class*="article"], [class*="Card"], [class*="story"], [data-testid*="search"]');
        const items = [];
        for (const card of cards) {
            ...
            const categoryEl = card.querySelector('[class*="category"], [class*="tag"], [class*="label"], [class*="topic"]');
...
(async () => {
  ...
  try {
    ...
    const url = `https://www.coindesk.com/search?s=${query}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} cryptocurrency news article results. For each get the title, author, publish date, category, and summary.`
    );
    ...
    fs.writeFileSync(path.join(outDir, "coindesk_search.py"), genPython(CFG, recorder));
    console.log("Saved coindesk_search.py");
  } finally {
