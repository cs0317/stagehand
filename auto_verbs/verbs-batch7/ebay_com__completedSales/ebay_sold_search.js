 ...
 *
 * Searches eBay for sold items and extracts:
 * title, sold_price, sold_date, condition.
 */
...
(async () => {
  ...
  try {
    ...
    const url = `https://www.ebay.com/sch/i.html?_nkw=${q}&LH_Complete=1&LH_Sold=1`;
    console.log(`Navigating to ${url}`);
    await page.goto(url, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(CFG.waits.page);

    const result = await stagehand.extract({
      instruction: `Extract the first ${CFG.maxResults} sold/completed items. For each get: title, sold price, sold date, and condition.`,
      schema: {
        type: "object",
        properties: {
          items: {
    ...
    console.log(`\nExtracted ${result.items?.length || 0} sold items`);
    for (const item of result.items || []) {
      console.log(`\n  Title:     ${item.title}`);
      console.log(`  Price:     ${item.sold_price}`);
      console.log(`  Sold Date: ${item.sold_date}`);
    ...
    fs.writeFileSync(path.join(__dirname, "ebay_sold_search.py"), pyCode);
    console.log("\nSaved ebay_sold_search.py");
  } finally {
