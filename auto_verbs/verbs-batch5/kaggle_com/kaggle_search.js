 ...
 *
 * Searches Kaggle for datasets and extracts title, author,
 * file size, downloads, and usability rating.
 */

// ── Configuration ────────────────────────────────────────────────────────────
const CFG = {
  url: "https://www.kaggle.com/datasets?search=climate+change",
...
(async () => {
  ...
  try {
    ...
    recorder.record("goto", { url: CFG.url, description: `Search Kaggle for "${CFG.searchQuery}" datasets` });

    const datasets = await page.evaluate((maxResults) => {
      const items = document.querySelectorAll("li.MuiListItem-root");
      const results = [];
      for (const item of items) {
        if (results.length >= maxResults) break;
        ...
        const title = lines[0];
        if (title === "more_horiz" || title.length < 3) continue;
        // Author line: "Author · Updated N ago"
        const authorLine = lines.find(l => l.includes("Updated"));
        const author = authorLine ? authorLine.split("\u00b7")[0].trim() : "";
        // Metadata line: "Usability X.X · N File(s) (type) · size · downloads · notebooks"
        const metaLine = lines.find(l => l.includes("Usability"));
        if (!metaLine) continue;
        const usabilityMatch = metaLine.match(/Usability\s+([\d.]+)/);
        const usability = usabilityMatch ? usabilityMatch[1] : "";
    ...
    recorder.record("extract", {
      instruction: "Extract dataset cards",
      description: `Extracted ${datasets.length} datasets`,
      results: datasets,
    });
    ...
    datasets.forEach((d, i) => {
      ...
      console.log(`      Author: ${d.author} | Size: ${d.file_size} | Downloads: ${d.downloads}`);
      console.log(`      Usability: ${d.usability_rating}`);
    });
    ...
    fs.writeFileSync(path.join(dir, "kaggle_search.py"), genPython(CFG, recorder));
