 ...
 *
 * Browses the NomadList front page and extracts
 * top-ranked cities with scores and key metrics.
 */
...
nomadlist.com – Top Cities for Digital Nomads
Extract top ${cfg.maxResults} cities
...
(async () => {
  ...
  try {
    ...
    recorder.record("goto", { url, description: "Navigate to NomadList front page" });

    const cities = await page.evaluate((maxResults) => {
      const lis = document.querySelectorAll('li[data-type="city"]');
      const results = [];
      for (let i = 0; i < Math.min(lis.length, maxResults); i++) {
        const li = lis[i];
    ...
    recorder.record("extract", {
      instruction: "Extract top cities",
      description: `Extracted ${cities.length} cities`,
      results: cities,
    });

    console.log(`\n📋 Found ${cities.length} cities:\n`);
    cities.forEach((c, i) => {
      console.log(`   ${i + 1}. ${c.city_name}, ${c.country}`);
      console.log(`      Overall: ${c.overall_score} | Cost: ${c.cost_per_month} | Internet: ${c.internet_speed}`);
      if (c.temperature) console.log(`      Temperature: ${c.temperature}`);
    });
    ...
    fs.writeFileSync(path.join(dir, "nomadlist_top.py"), pyCode);
