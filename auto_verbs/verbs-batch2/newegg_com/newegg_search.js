...
const CFG = { url: "https://www.newegg.com", query: "mechanical keyboard", maxResults: 5, waits: { page: 3000, type: 2000, search: 5000 } };

function genPython(cfg, recorder) { const ts = new Date().toISOString();
  return `"""Auto-generated – Newegg Product Search. Query: ${cfg.query}. Generated: ${ts}"""
...
def run(playwright: Playwright, query: str = "${cfg.query}", max_results: int = ${cfg.maxResults}) -> list:
    print(f"  Query: {query}\\n")
    port = get_free_port(); profile_dir = get_temp_profile_dir("newegg_com")
    ...
    try:
        ...
        si = page.locator('input[id="SearchBox2"], input[aria-label*="search" i], input[type="search"]').first
        ...
        try:
            sort_btn = page.locator('select[id*="sort"], button:has-text("Sort"), [class*="sort"]').first
            sort_btn.select_option(label="Best Sellers") if sort_btn.evaluate("el => el.tagName") == "SELECT" else sort_btn.click()
            page.wait_for_timeout(2000)
        except Exception: pass
        cards = page.locator('[class*="item-cell"], [class*="item-container"], div[class*="item-info"]')
        count = cards.count()
        for i in range(min(count, max_results)):
            card = cards.nth(i); name = price = rating = "N/A"
            try: name = card.locator('a[class*="item-title"], a[title]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: price = card.locator('li[class*="price-current"], [class*="price"], strong:has-text("$")').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: rating = card.locator('[class*="item-rating"], [aria-label*="rating"], [title*="out of"]').first.get_attribute("title", timeout=2000) or "N/A"
            except Exception: pass
            if name != "N/A": results.append({"name": name[:100], "price": price, "rating": rating}); print(f"  {len(results)}. {name[:80]} | {price} | {rating}")
...
async function main() { const recorder = new PlaywrightRecorder(); const llmClient = setupLLMClient("hybrid"); let stagehand;
  try { stagehand = new Stagehand({ env: "LOCAL", verbose: 0, llmClient, localBrowserLaunchOptions: { userDataDir: path.join(os.homedir(), "AppData", "Local", "Google", "Chrome", "User Data", "Default"), headless: false, viewport: { width: 1920, height: 1080 }, args: ["--disable-blink-features=AutomationControlled", "--disable-infobars", "--disable-extensions", "--start-maximized"] } });
    ...
    const listings = await stagehand.extract(`Extract up to ${CFG.maxResults} products with name, price, and rating.`, z.object({ products: z.array(z.object({ name: z.string(), price: z.string(), rating: z.string() })) }));
    recorder.record("extract", { instruction: "Extract products", results: listings });
    fs.writeFileSync(path.join(__dirname, "newegg_search.py"), genPython(CFG, recorder), "utf-8");
