...
const CFG = { url: "https://www.washingtonpost.com", query: "climate change", maxResults: 5, waits: { page: 3000, type: 2000, search: 5000 } };

function genPython(cfg, recorder) { const ts = new Date().toISOString();
  return `"""Auto-generated – Washington Post Article Search. Query: ${cfg.query}. Generated: ${ts}"""
...
def run(playwright: Playwright, query: str = "${cfg.query}", max_results: int = ${cfg.maxResults}) -> list:
    ...
    results = []
    try:
        page.goto(f"${cfg.url}/search/?query={query.replace(' ', '+')}")
        page.wait_for_load_state("domcontentloaded"); page.wait_for_timeout(3000)
        for sel in ["button#onetrust-accept-btn-handler", "button:has-text('Accept')", "button:has-text('Close')", "button:has-text('Continue')"]:
            try:
                btn = page.locator(sel).first
                if btn.is_visible(timeout=1500): btn.evaluate("el => el.click()"); page.wait_for_timeout(500)
            except Exception: pass
        cards = page.locator('[data-testid*="result"], article, div[class*="result-item"], div[class*="story-list-story"]')
        count = cards.count()
        for i in range(min(count, max_results)):
            card = cards.nth(i); title = date = author = "N/A"
            try: title = card.locator('h2, h3, a[data-testid*="title"], [class*="title"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: date = card.locator('time, [class*="date"], span[class*="timestamp"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: author = card.locator('[class*="author"], [class*="byline"], span[data-testid*="author"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            if title != "N/A": results.append({"title": title, "date": date, "author": author}); print(f"  {len(results)}. {title} | {date} | {author}")
        print(f"\\nFound {len(results)} articles:")
        for i, r in enumerate(results, 1): print(f"  {i}. {r['title']} ({r['date']}) by {r['author']}")
...
async function main() { const recorder = new PlaywrightRecorder(); const llmClient = setupLLMClient("hybrid"); let stagehand;
  try { stagehand = new Stagehand({ env: "LOCAL", verbose: 0, llmClient, localBrowserLaunchOptions: { userDataDir: path.join(os.homedir(), "AppData", "Local", "Google", "Chrome", "User Data", "Default"), headless: false, viewport: { width: 1920, height: 1080 }, args: ["--disable-blink-features=AutomationControlled", "--disable-infobars", "--disable-extensions", "--start-maximized"] } });
    ...
    await page.goto(`${CFG.url}/search/?query=${CFG.query.replace(/ /g, '+')}`);
    await page.waitForLoadState("domcontentloaded"); await page.waitForTimeout(CFG.waits.search);
    const { z } = require("zod/v3");
    const listings = await stagehand.extract(`Extract up to ${CFG.maxResults} articles with title, date, and author.`, z.object({ articles: z.array(z.object({ title: z.string(), date: z.string(), author: z.string() })) }));
    recorder.record("extract", { instruction: "Extract articles", results: listings });
    fs.writeFileSync(path.join(__dirname, "washingtonpost_search.py"), genPython(CFG, recorder), "utf-8");
