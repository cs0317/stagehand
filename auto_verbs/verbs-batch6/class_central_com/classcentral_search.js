...
class CourseRequest:
    query: str = "${cfg.query}"
    max_courses: int = ${cfg.maxCourses}
...
(async () => {
  ...
  try {
    const url = `https://www.classcentral.com/search?q=${encodeURIComponent(CFG.query)}`;
    console.log(`\n🌐 Searching: ${url}...`);
    await page.goto(url, { waitUntil: "domcontentloaded", timeout: 30000 });
    await page.waitForTimeout(CFG.waits.page);
    recorder.record("goto", { url, description: "Search Class Central" });

    const coursesData = await stagehand.extract(
      "extract up to 5 courses with course title, provider, institution, rating, and number of reviews"
    );
    console.log("\n📊 Courses:", JSON.stringify(coursesData, null, 2));
    recorder.record("extract", { instruction: "Extract courses", results: coursesData });
    ...
    fs.writeFileSync(path.join(dir, "classcentral_search.py"), pyCode);
