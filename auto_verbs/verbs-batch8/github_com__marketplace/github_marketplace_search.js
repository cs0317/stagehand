...
(async () => {
  ...
  try {
    ...
    const url = `https://github.com/marketplace?query=${query}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} marketplace tool results. For each get the tool name, publisher, category, description, pricing, number of installs, and rating.`
    );
    recorder.record("extract", "marketplace tools", JSON.stringify(data));
    console.log("Extracted:", JSON.stringify(data, null, 2));

    const outDir = __dirname;
    fs.writeFileSync(path.join(outDir, "recorded_actions.json"), JSON.stringify(recorder.actions, null, 2));
    fs.writeFileSync(path.join(outDir, "github_marketplace_search.py"), genPython(CFG, recorder));
    console.log("Saved github_marketplace_search.py");
  } finally {
