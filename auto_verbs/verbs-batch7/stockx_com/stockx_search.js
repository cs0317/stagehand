 ...
 *
 * Searches for sneakers: shoe name, lowest ask price.
 */

const CFG = {
  searchQuery: "Nike Air Jordan 1 Retro",
  maxResults: 5,
  waits: { page: 8000 },
};
...
def stockx_search(page: Page, request: SearchRequest) -> SearchResult:
    ...
    js_code = r\\"\\"\\"(max) => {
        ...
        for (let i = 0; i < lines.length; i++) {
            if (lines[i].match(/results for|Clear All/)) { startIdx = i + 1; break; }
        }
        ...
        while (i < lines.length && shoes.length < max) {
            ...
            while (i < lines.length) {
                const l = lines[i];
                if (l.startsWith('$') || l === '--') { price = l; i++; break; }
                i++;
            }
            while (i < lines.length && (lines[i] === 'Xpress Ship' || lines[i] === 'Sponsored')) i++;
            if (name && price) shoes.push({shoe_name: name, lowest_ask: price});
        }
        return shoes;
    }\\"\\"\\"
...
(async () => {
  ...
  try {
    ...
    const url = \`https://stockx.com/search?s=\${query.replace(/ /g, "+")}\`;
    console.log(\`Navigating to \${url}\`);
    await page.goto(url, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(CFG.waits.page);

    const result = await stagehand.extract({
      instruction: \`Extract the first \${CFG.maxResults} sneaker results. For each get: shoe name and lowest ask price.\`,
      schema: {
        type: "object",
        properties: {
          shoes: {
    ...
    console.log(\`\\nExtracted \${result.shoes?.length || 0} shoes\`);
    for (const s of result.shoes || []) {
      console.log(\`\\n  Name: \${s.shoe_name}\`);
      console.log(\`  Ask:  \${s.lowest_ask}\`);
    }

    recorder.actions.push({ type: "extract", data: result });
    fs.writeFileSync(path.join(__dirname, "recorded_actions.json"), JSON.stringify(recorder.actions, null, 2));
    const pyCode = genPython(CFG, recorder);
    fs.writeFileSync(path.join(__dirname, "stockx_search.py"), pyCode);
    console.log("\\nSaved stockx_search.py");
  } finally {
