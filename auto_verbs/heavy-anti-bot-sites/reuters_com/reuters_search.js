...
const CFG = { url: "https://www.reuters.com", query: "artificial intelligence", maxResults: 5, waits: { page: 3000, type: 2000, search: 5000 } };

function genPython(cfg, recorder) { const ts = new Date().toISOString();
  return `"""Auto-generated – Reuters Article Search. Query: ${cfg.query}. Generated: ${ts}"""
...
def run(playwright: Playwright, query: str = "${cfg.query}", max_results: int = ${cfg.maxResults}) -> list:
    print(f"  Query: {query}\\n")
    port = get_free_port(); profile_dir = get_temp_profile_dir("reuters_com")
    ...
    results = []
    try:
        page.goto(f"${cfg.url}/site-search/?query={query.replace(' ', '+')}")
        page.wait_for_load_state("domcontentloaded"); page.wait_for_timeout(3000)
        for sel in ["button#onetrust-accept-btn-handler", "button:has-text('Accept')", "button:has-text('I Agree')", "button:has-text('Close')"]:
            try:
                btn = page.locator(sel).first
                if btn.is_visible(timeout=1500): btn.evaluate("el => el.click()"); page.wait_for_timeout(500)
            except Exception: pass
        cards = page.locator('[class*="search-result"], article, li[class*="result"], div[class*="media-story"]')
        count = cards.count()
        for i in range(min(count, max_results)):
            card = cards.nth(i); title = date = summary = "N/A"
            try: title = card.locator('h3, a[data-testid*="Heading"], [class*="title"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: date = card.locator('time, [class*="date"], span[class*="timestamp"]').first.inner_text(timeout=2000).strip()
            except Exception: pass
            try: summary = card.locator('p, [class*="description"], [class*="snippet"]').first.inner_text(timeout=2000).strip()[:200]
            except Exception: pass
            if title != "N/A": results.append({"title": title, "date": date, "summary": summary}); print(f"  {len(results)}. {title} | {date}")
        print(f"\\nFound {len(results)} articles:")
        for i, r in enumerate(results, 1): print(f"  {i}. {r['title']} ({r['date']})\\n     {r['summary'][:100]}")
...
async function main() { const recorder = new PlaywrightRecorder(); const llmClient = setupLLMClient("hybrid"); let stagehand;
  try { stagehand = new Stagehand({ env: "LOCAL", verbose: 0, llmClient, localBrowserLaunchOptions: { userDataDir: path.join(os.homedir(), "AppData", "Local", "Google", "Chrome", "User Data", "Default"), headless: false, viewport: { width: 1920, height: 1080 }, args: ["--disable-blink-features=AutomationControlled", "--disable-infobars", "--disable-extensions", "--start-maximized"] } });
    ...
    await page.goto(`${CFG.url}/site-search/?query=${CFG.query.replace(/ /g, '+')}`);
    await page.waitForLoadState("domcontentloaded"); await page.waitForTimeout(CFG.waits.search);
    const { z } = require("zod/v3");
    const listings = await stagehand.extract(`Extract up to ${CFG.maxResults} articles with title, date, and summary.`, z.object({ articles: z.array(z.object({ title: z.string(), date: z.string(), summary: z.string() })) }));
    recorder.record("extract", { instruction: "Extract articles", results: listings });
    fs.writeFileSync(path.join(__dirname, "reuters_search.py"), genPython(CFG, recorder), "utf-8");
