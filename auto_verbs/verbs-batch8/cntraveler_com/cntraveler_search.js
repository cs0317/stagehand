...
(async () => {
  ...
  try {
    ...
    const url = `https://www.cntraveler.com/search?q=${query}`;
    await page.goto(url, { waitUntil: "domcontentloaded" });
    recorder.record("navigate", url, `Navigate to ${url}`);
    await page.waitForTimeout(CFG.waits.page);

    const data = await stagehand.extract(
      `Extract the first ${CFG.maxResults} travel article results. For each get the title, author, publish date, category, summary, and image URL.`
    );
    ...
    fs.writeFileSync(path.join(outDir, "cntraveler_search.py"), genPython(CFG, recorder));
    console.log("Saved cntraveler_search.py");
  } finally {
